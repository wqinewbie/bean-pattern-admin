<template>
  <div>
    <el-card shadow="never" style="margin-bottom:16px">
      <el-row :gutter="12">
        <el-col :span="7">
          <el-input v-model="q" placeholder="搜索用户ID/昵称/图纸名称" clearable @input="onSearchInput">
    ...
    <el-card shadow="never">
      <el-table :data="list" v-loading="loading" stripe>
        <el-table-column label="图纸信息" min-width="200">
          <template #default="{row}">
            <div style="display:flex;flex-direction:column;gap:4px">
              <div style="font-weight:700">{{ row.name || '未命名图纸' }}</div>
    ...
    <PreviewDialog v-model="previewVisible" :record-id="previewRecordId" type="box" />
  </div>
</template>
...
async function load() {
  loading.value = true
  try {
    const data = await request.get('/admin/user-boxes', {
      params: {
        page: page.value,
...
async function handleDelete(row) {
  try {
    await ElMessageBox.confirm(`确定要删除图纸"${row.name || '未命名'}"吗？`, '提示', {
      type: 'warning'
    })
    await request.delete(`/admin/user-boxes/${row.id}`)
